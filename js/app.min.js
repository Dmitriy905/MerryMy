...
window.addEventListener('load', () => {
  if (document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper, .in4-wrapper')) {
    setTimeout(() => {
      document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper, .in4-wrapper').classList.add('_loaded');
    }, 0);
  }
});
...
//= ================
// Menu
const iconMenu = document.querySelector('.icon-menu, .in1-header__icon, .in4-header__icon');
if (iconMenu != null) {
  const delay = 500;
  const menuBody = document.querySelector('.menu__body, .in1-menu__body, .in4-menu__body');
...
function body_lock_add(delay) {
  ...
  if (unlock) {
    ...
    for (let index = 0; index < lock_padding.length; index++) {
      const el = lock_padding[index];
      el.style.paddingRight = `${window.innerWidth - document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper, .in4-wrapper').offsetWidth}px`;
    }
    body.style.paddingRight = `${window.innerWidth - document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper, .in4-wrapper').offsetWidth}px`;
...
function popup_close(item, bodyUnlock = true) {
  if (unlock) {
    ...
    if (!document.querySelector('.menu__body._active, .in1-menu__body._active , .in4-menu__body._active') && bodyUnlock) {
...
// Invitation№3 =====================================
const in4Timer = document.querySelectorAll('.in4-timer');
if (in4Timer.length > 0) {
  const date = new Date('March 30 2024 00:00:00');
  function counts() {
    const now = new Date();
    gap = date - now;
