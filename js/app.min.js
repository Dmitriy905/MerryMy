...
//Invitation№3 =====================================
let slider_header = new Swiper('.in3-menu__list', {
	observer: true,
	observeParents: true,
	slidesPerView: 'auto',
	spaceBetween: 0,
...
window.addEventListener('load', () => {
  if (document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper')) {
    setTimeout(() => {
      document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper').classList.add('_loaded');
    }, 0);
  }
});
...
//= ================
// Menu
const iconMenu = document.querySelector('.icon-menu, .in1-header__icon');
if (iconMenu != null) {
  const delay = 500;
  const menuBody = document.querySelector('.menu__body, .in1-menu__body');
...
function body_lock_add(delay) {
  ...
  if (unlock) {
    ...
    for (let index = 0; index < lock_padding.length; index++) {
      const el = lock_padding[index];
      el.style.paddingRight = `${window.innerWidth - document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper').offsetWidth}px`;
    }
    body.style.paddingRight = `${window.innerWidth - document.querySelector('.wrapper, .in1-wrapper, .in2-wrapper, .in3-wrapper').offsetWidth}px`;
...
function popup_open(item, video = '') {
  ...
  if (curent_popup && unlock) {
    ...
    if (!document.querySelector('.menu__body._active, .in1-menu__body._active')) {
      body_lock_add(500);
    }
...
function popup_close(item, bodyUnlock = true) {
  if (unlock) {
    ...
    if (!document.querySelector('.menu__body._active, .in1-menu__body._active') && bodyUnlock) {
...
if (link) {
  ...
  const _loop7 = function _loop7(_index28) {
    const el = link[_index28];
    ...
    el.addEventListener('click', (e) => {
      if (document.querySelector('.page__block._active')) {
        menu_close();
        body_lock_remove(500);
      }
  ...
  window.addEventListener('scroll', (el) => {
    ...
    if (old_current_link) {
      for (let _index29 = 0; _index29 < old_current_link.length; _index29++) {
        const _el13 = old_current_link[_index29];
      ...
      if (block_item) {
        ...
        if (pageYOffset > block_offset - window.innerHeight / 3 && pageYOffset < block_offset + block_height - window.innerHeight / 3) {
          ...
          for (let _index31 = 0; _index31 < current_links.length; _index31++) {
            const current_link = current_links[_index31];
...
function fix_block(scr_fix_block, scr_value) {
  ...
  for (let index = 0; index < scr_fix_block.length; index++) {
    ...
    if (window_width > block_width) {
      if (item.offsetHeight < window_height - (header_height + 301)) {
        if (scr_value > offset(block).top - (header_height + 151)) {
          item.style.cssText = `position:fixed;bottom:auto;top:${header_height}px;width:${block.offsetWidth}px;left:${offset(block).left}px;`;
        } else {
          gotoRelative(item);
        }
        if (scr_value > (block.offsetHeight + offset(block).top) - (item.offsetHeight + (header_height + 152))) {
